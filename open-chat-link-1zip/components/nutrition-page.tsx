...
import { FOOD_DATABASE, FOOD_CATEGORIES, searchFoods } from '@/lib/food-data'
import { FoodItem, MealEntry } from '@/lib/types'
import { Plus, Search, X, ArrowLeft, Minus } from 'lucide-react'
  ...
  return (
    <>
      <div className="space-y-4 pb-20">
        {/* Calorie Header Card */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 p-5 text-white">
          <div className="absolute right-0 top-0 h-32 w-32 translate-x-8 -translate-y-8 rounded-full bg-white/10" />
          <div className="relative">
            <p className="text-sm font-medium text-blue-100">Daily Calories</p>
            <div className="mt-1 flex items-baseline gap-2">
              <span className="text-4xl font-bold">{totals.calories}</span>
              <span className="text-lg text-blue-200">/ {settings.calorieGoal}</span>
            </div>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-blue-900/50">
              <div
            ...
            <p className="mt-2 text-sm text-blue-200">
              {settings.calorieGoal - totals.calories > 0
                ? `${settings.calorieGoal - totals.calories} calories remaining`
                : 'Goal reached!'}
            </p>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {view === 'categories' && searchQuery.length < 2 && (
          <>
            <h3 className="mb-3 text-sm font-medium text-muted-foreground">Browse by Macro</h3>
