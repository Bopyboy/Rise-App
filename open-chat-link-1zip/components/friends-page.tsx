...
function FriendRow({
  ...
  const rank = getRankByScore(friend.riseScore)
    ...
    <div className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
      ...
      {!pending && (
        <div className="text-right">
          <p className="text-sm font-bold text-foreground">{friend.riseScore.toLocaleString()}</p>
...
function FriendProfileView({ friend, onBack }: { friend: Friend; onBack: () => void }) {
  const rank = getRankByScore(friend.riseScore)
  const nextRankScore = [500, 1500, 3000, 5000, 8000, 12000, 18000, 25000]
  const nextThreshold = nextRankScore.find(s => s > friend.riseScore) ?? 25000
  const progressToNext = Math.min(100, (friend.riseScore / nextThreshold) * 100)
    ...
    <div className="space-y-4 pb-24 pt-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-border bg-card p-3 text-center">
          <p className="text-xl font-bold text-foreground">{friend.riseScore.toLocaleString()}</p>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-muted-foreground">
            {(nextThreshold - friend.riseScore).toLocaleString()} pts to next rank
          </span>
        </div>
