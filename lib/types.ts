...
export const RANKS: RankInfo[] = [
  ...
  { name: 'Bronze', symbol: '🟤', color: '#cd7f32', minScore: 500, glowClass: 'rank-glow-bronze' },
  { name: 'Silver', symbol: '⚪', color: '#c0c0c0', minScore: 1500, glowClass: 'rank-glow-silver' },
  { name: 'Gold', symbol: '🟡', color: '#fbbf24', minScore: 3000, glowClass: 'rank-glow-gold' },
  { name: 'Platinum', symbol: '💠', color: '#67e8f9', minScore: 5000, glowClass: 'rank-glow-platinum' },
  { name: 'Diamond', symbol: '💎', color: '#38bdf8', minScore: 8000, glowClass: 'rank-glow-diamond' },
  { name: 'Master', symbol: '🔴', color: '#f87171', minScore: 12000, glowClass: 'rank-glow-master' },
  { name: 'Grandmaster', symbol: '🟣', color: '#c084fc', minScore: 18000, glowClass: 'rank-glow-grandmaster' },
  { name: 'Elite', symbol: '⚡', color: '#fbbf24', minScore: 25000, glowClass: 'rank-glow-elite' },
]
...
export function getEstimatedDaysToNextRank(score: number, dailyRate: number = 150): number {
  const pointsNeeded = getPointsToNextRank(score)
  if (pointsNeeded <= 0 || dailyRate <= 0) return 0
  return Math.ceil(pointsNeeded / dailyRate)
}

export function formatETA(days: number): string {
  if (days === 0) return 'Max rank achieved!'
  if (days <= 7) return `~${days} day${days === 1 ? '' : 's'} at current pace`
  if (days <= 30) return `~${Math.ceil(days / 7)} week${Math.ceil(days / 7) === 1 ? '' : 's'} at current pace`
  return `~${Math.ceil(days / 30)} month${Math.ceil(days / 30) === 1 ? '' : 's'} at current pace`
}
